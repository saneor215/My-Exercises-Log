...
export const StartRoutineModal: React.FC<StartRoutineModalProps> = ({
    isOpen, onClose, onSave, routines, bodyParts, exercises
}) => {
    ...
    useEffect(() => {
        if (selectedRoutine) {
            setEntriesData(selectedRoutine.exercises.map(() => ({ weight: '', reps: '', comment: '' })));
        } else {
            setEntriesData([]);
        }
    }, [selectedRoutine]);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-gray-800 rounded-2xl shadow-xl w-full max-w-3xl p-6 ring-1 ring-white/10 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <select value={selectedRoutineId} onChange={e => setSelectedRoutineId(e.target.value)} className="bg-gray-700 text-white px-3 py-2 rounded-lg">
                        {routines.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                    </select>
                    <input type="number" min="1" value={week} onChange={e => setWeek(e.target.value)} placeholder="رقم الأسبوع" className="bg-gray-700 text-white px-3 py-2 rounded-lg" required />
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 -mr-2 space-y-4 border-t border-gray-700 pt-4">
                    {selectedRoutine && selectedRoutine.exercises.map((re, index) => {
                         ...
                         if (!data) return null; // FIX: Prevents crash from race condition
                         return (
                            <div key={index} className="bg-gray-700/50 p-3 rounded-lg">
                                <div className="flex items-center gap-4 mb-3">
                                    <img src={details?.image} alt={re.exerciseName} className="w-16 h-16 rounded-md object-cover" />
                                    <p className="font-semibold text-lg text-white">{re.exerciseName}</p>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    <input type="number" value={data.weight} onChange={e => handleDataChange(index, 'weight', e.target.value)} placeholder="الوزن (كجم)" className="w-full bg-gray-600 text-white px-3 py-2 rounded-md" />
                                    <input type="number" value={data.reps} onChange={e => handleDataChange(index, 'reps', e.target.value)} placeholder="التكرارات" className="w-full bg-gray-600 text-white px-3 py-2 rounded-md" />
                                    <textarea value={data.comment} onChange={e => handleDataChange(index, 'comment', e.target.value)} placeholder="تعليق (اختياري)" rows={1} className="md:col-span-2 w-full bg-gray-600 text-white px-3 py-2 rounded-md"></textarea>
                                </div>
                            </div>
                         );
                    })}
                ...
                <div className="flex justify-end gap-4 pt-4 mt-4 border-t border-gray-700">
                    ...
                    <button onClick={handleSaveAll} className="px-6 py-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-semibold">حفظ كل التمارين</button>
                </div>
            </div>
        </div>
    );
